mergeInto(LibraryManager.library, {
    ...
    PostTokensReq: function(path, value6, hash, time, timeStamp, objectName, callback, fallback) {
        var parsedPath = Pointer_stringify(path);
        var parsedHash = Pointer_stringify(hash);
        var parsedValue = Pointer_stringify(time);
        var a_TokensReq, b_TxnHash, c_TR_Updated;
        var parsedObjectName = Pointer_stringify(objectName);
        var parsedCallback = Pointer_stringify(callback);
        var parsedFallback = Pointer_stringify(fallback);
        ...
        try {
            ...
            firebase.database().ref(parsedPath).child("c_TokensReq").update({
                a_TokensReq: value6,
                b_TxnHash: parsedHash,
                c_TR_Updated: parsedValue,
    ...
    PostTokensClaim: function(path, value7, value8, objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.database().ref(parsedPath).child("d_TokensClaim").update({
    ...
    PostHistory: function(path, category, year, month, day, time, value, callback, fallback) {
        var parsedPath = Pointer_stringify(path);
        var parsedCategory = Pointer_stringify(category);
        ...
        try {
                ...
                firebase.database().ref(parsedPath).child(parsedCategory).child("z_History").child(year).child(month).child(day).update({
                [parsedTime]: value,
            });
